...
which.min(student1)
```
What I want to now drop (i.e. exclude) this lowest score from my mean() calculation.
...
```
Now we are essentialy there with out working snippet!
```{r}
x <- student3
x[is.na(x)] <- 0
mean( x[-which.min(x)])
...
Now finally we can use our funciton on our "real" whole class data
